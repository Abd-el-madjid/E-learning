...
">
		...
		<div class="container"><h2>List of Users </h2>
			...
			<ul id="responsive-table" class="responsive-table">
			  ...
			  <li class="table-row">
				...
				<div class="col col-2"  data-label="Email"> ${listOfUsers[i].email}</div>
				<div class="col col-3" data-label="Fisrt Name"> ${listOfUsers[i].firstname}</div>
				<div class="col col-4" data-label="Last Name"> ${listOfUsers[i].lastname}</div>
				<div class="col col-5" data-label="Password"> ${listOfUsers[i].password}</div>
				<div class="col col-6" data-label="Type">${listOfUsers[i].type}</div>
				<div class="col col-7" data-label="Gendre"> ${listOfUsers[i].sex}</div>
				<div class="col col-8" data-label="Update"> 
				<form id="update" action="beforeabdate" method="post" >
		<input name="email" value="${listOfUsers[i].email}" hidden> 
		<input name="sex" value="${listOfUsers[i].sex}" hidden> 
		<input name="type" value="${listOfUsers[i].type}" hidden>
		<input name="password" value="${listOfUsers[i].password}" hidden>
		<input name="lname" value="${listOfUsers[i].lastname}" hidden>
		<input name="fname" value="${listOfUsers[i].firstname}" hidden>
				<input class="btn" type="submit" value="Update">
				</form>	
				</div>
				
				<div class="col col-9" data-label="Delet">
					 <form id="deletform" action="DeletUserServlet" method="post" >
					
				 <input name="type" value="listOfUsers" hidden>
					 <input name="k" value="${listOfUsers[i].email}" hidden>
